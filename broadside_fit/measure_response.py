...
freqs_oi = rt.freq_calc(1024, 1.0/1.5e9/2.0)
...
def disc_residuals(coeffs, y, t):
    ...
    res *= freqs_oi # Weighted by frequency
    res = np.fft.irfft(res)
    res *= np.std(res)
    res *= np.std(model)
    return res

h_len_before_trunc = len(h_fft)
h_fft_trunc = h_fft[:513]

x1_fft = h_fft_trunc;
x1 = np.fft.irfft(x1_fft)    
target = np.fft.irfft(h_fft_trunc)
...
h_fft = np.fft.rfft(disc_model(steps, x))
h_fft = np.append(h_fft, np.zeros(h_len_before_trunc - len(h_fft)))
